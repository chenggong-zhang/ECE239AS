...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        encoder_in = torch.cat((x_flat,c),dim=1)
        encoder_out = self.encoder(encoder_in)
        mu = self.mu_layer(encoder_out)
        logvar = self.logvar_layer(encoder_out)
        z = reparametrize(mu, logvar)
        decoder_in = torch.cat((z,c),dim=1)
...
def reparametrize(mu, logvar):
    ...
    scaled = epsilon * torch.exp(0.5 * logvar)
    z = mu + scaled
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
...
def loss_function(x_hat, x, mu, logvar):
    ...
    # Replace "pass" statement with your code
    N = x.shape[0]
    rec_term = torch.nn.functional.binary_cross_entropy(x_hat, x, reduction = "sum")
    kl_div = -0.5*torch.sum(1+logvar - torch.pow(mu,2) - torch.exp(logvar))
    loss = (rec_term + kl_div) / N
    ################################################################################################
    #                            END OF YOUR CODE                                                  #
    ################################################################################################
    return loss
