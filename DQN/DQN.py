...
class DQN:
    ...
    def _optimize_model(self):
        ...
        if len(self.replay_buffer) < self.batch_size*10:
            #warm start until we can get a sample from the replay buffer
            return False,0
        state,actions,rewards,next_state,done = self.replay_buffer.sample(self.batch_size)
        state = state.to(self.device)
        next_state = next_state.to(self.device)
        actions_indexs = actions.to(self.device)
        rewards = rewards.to(self.device) 
        done = done.to(self.device) 
        q_values = self.model(state) 
        q_values = q_values.gather(1, actions_indexs.unsqueeze(1)).squeeze(1) 
        with torch.no_grad():
...
class HardUpdateDQN(DQN):
    ...
    def __init__(self,env,model,model_kwargs:dict = {},
        ...
        self.target_model = model(
            self.observation_space,
            self.env.action_space.n,**model_kwargs).to(self.device)
...
class SoftUpdateDQN(HardUpdateDQN):
    ...
    def _update_model(self):
        if self.i_update % self.update_freq == self.update_freq - 1:
